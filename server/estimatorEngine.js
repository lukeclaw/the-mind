...
function buildEffectiveInputs(inputs) {
...
function estimateScenario(modelInput, rawInputs, options = {}) {
...
function estimateModelEnvelope(inputs) {
    return MODEL_CATALOG.map((model) => estimateScenario(model, inputs));
}

function recommendConfigs(inputs) {
...
function recommendMinimumBuild(inputs) {
...
function feasibilityLabel(code) {
    const map = {
...
function mergeEstimateInputs(payload) {
    const merged = {
        ...DEFAULT_INPUTS,
        ...(payload || {})
    };

    // Allow spec-style request shapes
    if (payload && payload.workload) {
        if (payload.workload.context_tokens != null) merged.contextLength = Number(payload.workload.context_tokens);
        if (payload.workload.output_tokens != null) merged.outputTokens = Number(payload.workload.output_tokens);
        if (payload.workload.concurrency != null) merged.concurrentUsers = Number(payload.workload.concurrency);
    }

    if (payload && payload.model_id) merged.modelId = payload.model_id;
    if (payload && payload.runtime_id) merged.runtimeId = payload.runtime_id;
    if (payload && payload.quant_id) merged.quantId = payload.quant_id;
